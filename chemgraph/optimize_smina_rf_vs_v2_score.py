...
class BARewardMolecule(molecules_mdp.Molecule):
  ...
  def _reward(self):
    ...
    Returns:
      ...
      final reward: RF Score VS V2 score (An estimated of the binding affinity)
    """
    ...
    if self._counter >= self.max_steps: # terminal state
      ...
      # docking with SMINA
      docking_cmd ="smina -r 6NM8-receptor.pdb -l ligand.pdbqt --autobox_ligand 6NM8-KSD.pdb --num_modes 1 -o ligand_out.sdf  > log_docking.txt"
      os.system(docking_cmd)
      
      # Recoring with RFScore Vs V2
      rf_score_cmd = './rf-score-vs --receptor 6NM8-receptor.pdb ligand_out.sdf -O ligand_out_rescored.csv'
      os.system(rf_score_cmd)


     # parsing RFScore VS V2 score from the resulting output from csv file
      try:
        data = pd.read_csv('ligand_out_rescored.csv')
      except:
        return 0.0
      rf_score_v2 = round(float(data['RFScoreVS_v2']), 3)
      print("RF Score V2 VS  value: "+str(round(rf_score_v2,2))+'\t'+self._state)

    # record a optimized result with the SMILES, RF Score VS V2 docking score, SA score,
    # and QED score.
    with open('./optimized_result_of_rf_score_total_pdl1_bms_202.txt', 'a') as f2:
      f2.write(self._state+'\t'+str(rf_score_v2)+'\t'+str(sa_norm)+'\t'+str(qed)+'\n')

    # we use the positive values of the rf score VS v2 because the more positive score
    # the better.
    return round(rf_score_v2, 2)
