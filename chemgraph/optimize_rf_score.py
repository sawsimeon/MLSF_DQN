...
class BARewardMolecule(molecules_mdp.Molecule):
  ...
  def _reward(self):
    ...
    if self._counter >= self.max_steps: # terminal state
      ...
      # docking
      docking_cmd ="qvina2 --config config.txt --num_modes=1 > log_docking.txt"
      os.system(docking_cmd)

      cvt_cmd_sdf = 'obabel ligand_out.pdbqt -O ligand_out.sdf > /dev/null 2>&1'
      os.system(cvt_cmd_sdf)

      rf_score_cmd = './rf-score-vs --receptor 3zosA_prepared.pdbqt ligand_out.sdf -O ligand_out_rescored.csv'
      ...
      try:
        ...
        return 0.0
      docking_score = round(float(data['RFScoreVS_v2']), 3)
      print("RF Score VS  value: "+str(round(docking_score,2))+'\t'+self._state)

    # record a optimized result with the SMILES, docking score, SA score,
    # and QED score.
    with open('./optimized_result_total_RF_Score.txt', 'a') as f2:
      f2.write(self._state+'\t'+str(docking_score)+'\t'+str(sa_norm)+'\t'+str(qed)+'\n')

    # we use the positive values of the RF_Score score because the higher RF  score
    # the better.
